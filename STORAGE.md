...
1. **Instance Storage (`e.storage().instance()`)**:
   - Shared contract-level configuration and global protocol parameters (`Admin`, `AdminPubKey`, `Name`, `Symbol`, `MigrationVersion`, …).
...
| `LatestPeriod(Address)` | Persistent | `u64` | 1 Year (`6,307,200` ledgers) | 8 bytes | Highest period index currently owned by a user. |
| `WrapPeriods(Address)` | Persistent | `Vec<u64>` | 1 Year (`6,307,200` ledgers) | Bounded by user wrap count | Legacy period index (collapsed into `UserPeriods`). |
...
| `UserPeriods(Address)` | Persistent | `Vec<u64>` | 1 Year (`6,307,200` ledgers) | Bounded by user history | Single unified index of periods currently owned by the user (updated on mint, transfer, and burn). |
...
| `LastUpdated(Address)` | Persistent | `u64` | 1 Year (`6,307,200` ledgers) | 8 bytes | Ledger timestamp of last user state modification (mint, revoke, transfer, burn). |
